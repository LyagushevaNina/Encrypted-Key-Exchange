...
public partial class AliceForm : Form
{
	...
	private async void StartListening ()
	{
		...
		client = await listener.AcceptTcpClientAsync().ConfigureAwait(true);
		...
		BigInteger bobPublicKey = ReceiveBigInteger(stream);
		AppendMessage($"Public key received: {bobPublicKey}");

		sharedSecret = BigInteger.ModPow(bobPublicKey, privateKey, p);
		...
		while (true)
		{
			...
			try
			{
				...
				if (bytesRead > 0)
				{
					...
					AppendMessage($"Bob: {decryptedMessage}");
				}
			}
			catch (Exception ex)
			{
